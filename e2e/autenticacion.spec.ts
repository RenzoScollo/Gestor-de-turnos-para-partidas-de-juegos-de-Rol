...
test('catálogo: editar clase y tienda, quitar vínculo y respetar dependencias al eliminar', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Nuevo objeto', exact: true }).click();
  await page.getByLabel('Nombre', { exact: true }).fill('Objeto catálogo E2E');
  await page.getByLabel('Descripción', { exact: true }).fill('Descripción del objeto de catálogo');
  await page.getByLabel('Tipo', { exact: true }).fill('Arma');
  await page.getByRole('spinbutton', { name: 'Valor', exact: true }).fill('0.5');
  await page.getByRole('button', { name: 'Guardar', exact: true }).click();
  await expect(page.getByText('Ingresá un valor entero mayor o igual a cero.')).toBeVisible();
  await page.getByRole('spinbutton', { name: /^Valor/ }).fill('60');
  await page.getByRole('combobox', { name: /^Tienda/ }).selectOption({ label: 'Tienda catálogo E2E' });
  await page.getByRole('button', { name: 'Guardar', exact: true }).click();
  const objectCard = page.locator('.objeto-card').filter({ hasText: 'Objeto catálogo E2E' });
  await expect(objectCard).toContainText('Valor 60');
  await objectCard.getByRole('button', { name: 'Editar', exact: true }).click();
  await page.getByLabel('Nombre', { exact: true }).fill('Objeto editado E2E');
  await page.getByLabel('Tipo', { exact: true }).fill('Reliquia');
  await page.getByRole('spinbutton', { name: 'Valor', exact: true }).fill('75');
  await page.getByRole('spinbutton', { name: 'Nivel', exact: true }).fill('2');
  await page.getByRole('checkbox', { name: /Objeto Único/ }).check();
  await page.getByRole('button', { name: 'Guardar', exact: true }).click();
  await expect(page.getByRole('status')).toContainText('Objeto actualizado correctamente.');
  await page.reload();
  const editedObject = page.locator('.objeto-card').filter({ hasText: 'Objeto editado E2E' });
  await expect(editedObject).toContainText('Nivel 2 · Valor 75');
  await expect(editedObject).toContainText('Único');
  await page.getByRole('searchbox', { name: 'Buscar objetos por nombre o descripción' }).fill('no existe este objeto');
  await expect(page.locator('.objeto-card')).toHaveCount(0);
  await page.getByRole('searchbox', { name: 'Buscar objetos por nombre o descripción' }).fill('Objeto editado');
  await page.getByRole('combobox', { name: 'Filtrar objetos por tipo' }).selectOption('Reliquia');
  await expect(editedObject).toBeVisible();
  await editedObject.getByRole('button', { name: /Objeto editado E2E/ }).click();
  await expect(page.locator('.objeto-detalle')).toContainText('Tienda catálogo E2E');
  await page.getByRole('searchbox', { name: 'Buscar objetos por nombre o descripción' }).fill('');
  await page.getByRole('combobox', { name: 'Filtrar objetos por tipo' }).selectOption('');
  page.once('dialog', dialog => dialog.accept());
  await editedObject.getByRole('button', { name: 'Eliminar', exact: true }).click();
  await expect(page.getByRole('status')).toContainText('Objeto eliminado correctamente.');
  await page.reload();
  await expect(page.getByText('Cargando objetos...', { exact: true })).toHaveCount(0);
  await expect(editedObject).toHaveCount(0);
  await page.goto('/classes');
  page.once('dialog', dialog => dialog.accept());
...
test('dos usuarios completan juego, recompensas, karma y comercio con inventarios', async ({ page: host, browser }) => {
  ...
  try {
    ...
    await expect(player.getByRole('button', { name: 'Nuevo objeto', exact: true })).toHaveCount(0);
    await expect(player.getByRole('button', { name: 'Editar', exact: true })).toHaveCount(0);
    await expect(player.getByRole('button', { name: 'Eliminar', exact: true })).toHaveCount(0);
